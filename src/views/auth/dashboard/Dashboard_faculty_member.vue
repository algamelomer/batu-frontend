...
const handleClick = () => {
    router.push({ name: 'DashboardUpload', params: { category: 'facultymember' } });
};
const handleEdit = (id) => {
    router.push({ name: 'DashboardEdit', params: { category: 'facultymember', id: id } });
}


const DeleteItem = async (id) => {
    await authstore.handleFacultieMemberDelete(id)
    fetchData();
}
</script>